'use client';

import { Rows3, LayoutGrid } from 'lucide-react';
import type { ViewMode } from '@/lib/useViewMode';

interface ViewToggleProps {
  mode: ViewMode;
  onChange: (mode: ViewMode) => void;
  height?: number;
}

export default function ViewToggle({ mode, onChange, height = 42 }: ViewToggleProps) {
  const btn = (active: boolean) => ({
    background: active ? 'var(--surface)' : 'transparent',
    color: active ? 'var(--accent)' : 'var(--text-muted)',
    boxShadow: active ? '0 1px 2px rgba(0,0,0,0.06)' : 'none',
  });
  const innerHeight = height - 10;

  return (
    <div className="inline-flex items-center rounded-xl gap-0.5 flex-shrink-0"
      style={{ background: 'var(--surface-2)', border: '1.5px solid var(--border)', height, padding: '0 4px', boxSizing: 'border-box' }}>
      <button
        onClick={() => onChange('table')}
        className="flex items-center gap-1.5 px-2.5 rounded-lg text-xs font-semibold transition-colors"
        style={{ ...btn(mode === 'table'), height: innerHeight }}
        title="Tampilan tabel"
      >
        <Rows3 size={14} /> <span className="hidden sm:inline">Tabel</span>
      </button>
      <button
        onClick={() => onChange('card')}
        className="flex items-center gap-1.5 px-2.5 rounded-lg text-xs font-semibold transition-colors"
        style={{ ...btn(mode === 'card'), height: innerHeight }}
        title="Tampilan kartu"
      >
        <LayoutGrid size={14} /> <span className="hidden sm:inline">Kartu</span>
      </button>
    </div>
  );
}
